...
void sortedListInsert(struct Node** hRef, struct Node* newNode){ 
    struct Node* current; 
    if (*hRef == NULL || (*hRef)->number >= newNode->number){ 
        newNode->next = *hRef; 
        *hRef = newNode; 
    }else{ 
        current = *hRef; 
...
void sortList(struct Node **hRef){ 
    struct Node *sortedList = NULL; 
  
    struct Node *current = *hRef; 
    while (current != NULL){ 
        struct Node *next = current->next; 
        sortedListInsert(&sortedList, current); 
        current = next; 
    } 
    *hRef = sortedList; 
} 
